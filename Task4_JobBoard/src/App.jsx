...
function App() {
  ...
  const addJob = (job) => {
    fetch('http://localhost:5000/jobs', {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error('Server route not found (404) or error occurred');
        }
        return res.json();
      })
      .then((newJobFromServer) => {
        // Adds the new job to the top of the list immediately
        setJobs((prevJobs) => [newJobFromServer, ...prevJobs]);
      })
      .catch((err) => {
        console.error("Error adding job:", err);
        alert("Could not post job. Is the server running?");
      });
  };
  const deleteJob = (id) => {
